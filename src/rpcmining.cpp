...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    GenerateROIcoins(fGenerate, pwalletMain, nGenProcLimit);
...
// NOTE: Unlike wallet RPC (which use ROI values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "See https://en.roicoin.it/wiki/BIP_0022 for full specification.\n"
        ...
        (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 5))
    {
        ...
        if (chainActive.Height()+1 >= 1 && chainActive.Height()+1<7884000) {
            LogPrintf("Creating Block...\n");
