...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-checkblocks=<n>", strprintf(_("How many blocks to check at startup (default: %u, 0 = all)"), 720));
    strUsage += HelpMessageOpt("-checklevel=<n>", strprintf(_("How thorough the block verification of -checkblocks is (0-4, default: %u)"), 3));
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "ROIcoin.conf"));
    if (mode == HMM_ROICOIND)
    {
#if !defined(WIN32)
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 3377, 33770));
    ...
    if (showDebug)
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf("Fees (in ROI/Kb) smaller than this are considered zero fee for transaction creation (default: %s)",
    ...
    strUsage += HelpMessageGroup(_("RPC SSL options: (see the ROIcoin Wiki for SSL setup instructions)"));
...
bool InitSanityCheck(void)
{
    if(!ECC_InitSanityCheck()) {
        InitError("OpenSSL appears to lack support for elliptic curve cryptography. For more "
                  "information, visit https://en.roicoin.it/wiki/OpenSSL_and_EC_Libraries");
        return false;
    }
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. ROIcoin is shutting down."));
    ...
    try {
        ...
        if (!lock.try_lock())
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. ROIcoin Core is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. ROIcoin Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    bool fProxy = false;
    if (mapArgs.count("-proxy")) {
        addrProxy = proxyType(CService(mapArgs["-proxy"], 3377), GetBoolArg("-proxyrandomize", true));
    ...
    if (!(mapArgs.count("-onion") && mapArgs["-onion"] == "0") &&
        ...
        if (!mapArgs.count("-onion"))
            ...
            addrOnion = proxyType(CService(mapArgs["-onion"], 3377), GetBoolArg("-proxyrandomize", true));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                if (fHavePruned && GetArg("-checkblocks", 720) > MIN_BLOCKS_TO_KEEP) {
                    LogPrintf("Prune: pruned datadir may not have more than %d blocks; -checkblocks=%d may fail\n",
                        MIN_BLOCKS_TO_KEEP, GetArg("-checkblocks", 720));
                }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of ROIcoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart ROIcoin Core to complete") << "\n";
    ...
    if (pwalletMain)
        GenerateROIcoins(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", -1));
#endif
